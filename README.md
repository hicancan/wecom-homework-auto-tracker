...
脚本会 fail-fast：标题、列名、空 `提交序号`、空 `提交内容`、非法后缀契约都会直接失败。上传文件后缀不在 `提交内容` 契约内时，记录为 `后缀格式无效`，不计入最终有效提交。
...
- 最新提交后缀无效时，该业务键统计为 `后缀格式无效`，不用旧有效版本充当本次有效提交。
- 已发布提交序号会保留 `统计截止时间`；统计截止时间只用于区分截止前有效提交与截止后补交。
- 看板主提交率和 zip 均使用当前最终有效文件；补交计入最终提交率，并用蓝色区分。
...
单个提交序号 JSON 使用 `提交序号` 作为主字段，并包含 `统计截止时间`、最终提交率、补交状态与后缀格式无效状态。
