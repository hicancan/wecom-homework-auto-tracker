...
- `archive_manifest.json` 是本地权威索引。
- 企业微信同步目录有同一业务键的新提交时，替换 active 文件。
- 企业微信同步目录源文件已删除，但归档里已有 active 文件时，继续保留本地归档。
- 最新提交后缀无效时，该业务键统计为无效，不用旧 active 文件充当有效提交。
- 已发布提交序号会保留 `统计截止时间`；看板和 zip 只统计截止时间之前的记录，避免 Excel 里的后续补交污染旧云端面板口径。
- 迁移前已经打包在本地的文件会作为本地归档基线保留，即使源附件已经删除，也不会被晚到 Excel 记录误判为缺失。
...
- `output_filename_templates`: 输出命名模板，默认 `{student_no}{student_name}{ext}`。
...
- 云端文件删了会不会丢：不会。只要本地 `archive_manifest.json` 里已有 active 文件，后续统计会保留归档基线。
